...
static __forceinline int compileProgram(const char *fragment) {
  ...
  //dirt: get program log
  int is_linked = 0;
  glGetProgramiv(pid, GL_LINK_STATUS, &is_linked);
  if (is_linked == 0){
    int max_len = 0;
    glGetProgramiv(pid, GL_INFO_LOG_LENGTH, &max_len);
    char prog_log[max_len];
    glGetProgramInfoLog(pid, max_len, &max_len, &prog_log[0]);
    printf("prog log: %s\n", prog_log);
  }
...
static void paint(int prog, int src_tex, int dst_fb, float time) {
  ...
  // dirt: what are we sending to b v c a d
...
// dirt: add opengl debug message callback
void GLAPIENTRY
